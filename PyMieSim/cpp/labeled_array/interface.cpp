...
public:
    ...
    py::array to_numpy() const { return values_; }

    py::array as_numpy() const { return values_; }
    ...
    py::object as_dataframe() const {
        ...
        py::object dataframe = pandas.attr("DataFrame")(columns);
    ...
    LabeledArray isel(py::dict indexers) const {
        ...
        for (std::size_t axis = 0; axis < dims_.size(); ++axis) {
            ...
            for (ssize_t i = 0; i < selectors.size(); ++i) {
...
PYBIND11_MODULE(labeled_array, module)
{
    ...
    py::class_<LabeledArray>(module, "LabeledArray")
        ...
        .def("__array__", [](const LabeledArray& self, py::object) { return self.to_numpy(); }, py::arg("dtype") = py::none())
