...
double BaseDetector::get_norm1_squared(const std::vector<T> &array) const
{
    T sum  = 0.0;
...
template <class T> inline
double BaseDetector::get_norm2_squared(const std::vector<T> &array) const
{
    T sum  = 0.0;
...
template <class T> inline
void BaseDetector::square_array(std::vector<T> &array)
{
    for (T &v : array)
        v = pow( abs(v), 2);
}

void BaseDetector::parse_mode(const std::string& mode_number) {
...
void BaseDetector::apply_interface_transmission_to_fields(
