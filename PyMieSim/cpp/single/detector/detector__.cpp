...
std::vector<double> Photodiode::get_poynting_field(const BaseScatterer& scatterer, double distance) const {
    ...
    for (size_t i = 0; i < Ephi.size(); ++i) {
        ...
        magnetic_field_norm = (electric_field_norm / Constants::LIGHT_SPEED);  // units of Tesla
        poynting[i] = (
            Constants::EPSILON0 *
            std::pow(Constants::LIGHT_SPEED, 2) *
            electric_field_norm *
            magnetic_field_norm
        );  // units of W/m^2
    }
...
double Photodiode::get_energy_flow(const BaseScatterer& scatterer) const {
    ...
    return trapz(poynting_vector, dA); // units of W
}
...
void IntegratingSphere::apply_interface_transmission_to_fields(
    std::vector<complex128>& theta_field,
    std::vector<complex128>& phi_field
) const
{
    // Minimal scalar approximation:
    // You already know this is not fully correct for s/p, but this is consistent with your current approach.
    const size_t n = theta_field.size();
    if (this->interface_t_s.size() != n || this->interface_t_p.size() != n) {
        throw std::runtime_error("Interface transmission coefficients not initialized or size mismatch.");
    }

    for (size_t i = 0; i < n; ++i) {
        const double t = std::sqrt(0.5 * (
            this->interface_t_s[i] * this->interface_t_s[i]
            + this->interface_t_p[i] * this->interface_t_p[i])
        );
        theta_field[i] *= t;
        phi_field[i]   *= t;
    }
...
{
    auto [phi_field, theta_field] = scatterer.compute_unstructured_farfields(this->fibonacci_mesh, 1.0);
    ...
    if (!std::isnan(this->polarization_filter)) {
        ...
        double sum_phi   = 0.0;
        for (size_t i = 0; i < theta_field.size(); ++i) {
            sum_theta += std::norm(theta_field[i]);
            sum_phi   += std::norm(phi_field[i]);
        }
...
std::vector<double> IntegratingSphere::get_poynting_field(
...
) const
{
    ...
    // this->apply_interface_transmission_to_fields(Etheta, Ephi);

    std::vector<double> poynting(Ephi.size());

    for (size_t i = 0; i < Ephi.size(); ++i) {
        const double fields_squared = std::norm(Ephi[i]) + std::norm(Etheta[i]);
        poynting[i] = (
            // scatterer.medium_refractive_index *
            Constants::EPSILON0 *
            Constants::LIGHT_SPEED *
...
double IntegratingSphere::get_energy_flow(const BaseScatterer& scatterer) const
{
    ...
    return trapz(poynting_vector, dA);
}
