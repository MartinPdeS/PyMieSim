...
void register_base_scatterer(pybind11::module_& module) {
    ...
    pybind11::class_<BaseScatterer, std::shared_ptr<BaseScatterer>>(module, "BASESCATTERER")
        ...
        .def(
            "get_s1s2",
            [ureg](BaseScatterer& self, const py::object &phi)
            {
                py::object Angle = py::module_::import("PyMieSim.units").attr("Angle");
                Angle.attr("check")(phi);
        ...
        .def(
            "get_farfields",
            [ureg](BaseScatterer& self, const py::object& phi, const py::object& theta, const py::object& distance)
            {
                py::object Angle = py::module_::import("PyMieSim.units").attr("Angle");
                py::object Length = py::module_::import("PyMieSim.units").attr("Length");

                Angle.attr("check")(phi);
                Angle.attr("check")(theta);
                Length.attr("check")(distance);
                ...
                auto [theta_field, phi_field] = self.compute_unstructured_farfields(phi_values, theta_values, distance_value);
        ...
        .def(
            "get_full_farfields",
            [ureg](BaseScatterer& self, const size_t sampling, py::object distance) {

                double distance_value = distance.attr("to")(ureg.attr("meter")).attr("magnitude").cast<double>();

                auto [phi_field_mesh, theta_field_mesh, theta_mesh, phi_mesh] = self.compute_full_structured_farfields(sampling, distance_value);
                return std::make_tuple(
        ...
        .def(
            ...
            [ureg](BaseScatterer& self, const py::object phi, const py::object theta, const py::object distance) {
                ...
                return std::make_tuple(
                    ...
                    vector_move_from_numpy(Q, {Q.size()}),
                    vector_move_from_numpy(U, {U.size()}),
                    vector_move_from_numpy(V, {V.size()})
                );
            },
            pybind11::arg("phi"),
            pybind11::arg("theta"),
