...
PYBIND11_MODULE(interface_source, module) {
    ...
    py::class_<Planewave, BaseSource, std::shared_ptr<Planewave>>(module, "PLANEWAVE")
        .def(
            "__init__",
            [](Planewave &instance, py::object wavelength, std::vector<complex128> jones_vector, py::object amplitude) {
                py::object ureg = get_shared_ureg();
                double wavelength_meter = (wavelength.attr("to")(ureg.attr("meter"))).attr("magnitude").cast<double>();
                double amplitude_vpm = (amplitude * ureg.attr("volt/meter")).attr("to")(ureg.attr("volt/meter")).attr("magnitude").cast<double>();
                new (&instance) Planewave(wavelength_meter, jones_vector, amplitude_vpm);
            },
            py::arg("wavelength"),
            py::arg("jones_vector"),
            py::arg("amplitude"),
    ...
    py::class_<Gaussian, BaseSource, std::shared_ptr<Gaussian>>(module, "GAUSSIAN")
        .def(
            "__init__",
            [](Gaussian &instance, py::object wavelength, std::vector<complex128> jones_vector, py::object NA, py::object optical_power) {
                py::object ureg = get_shared_ureg();

                double wavelength_meter = (wavelength.attr("to")(ureg.attr("meter"))).attr("magnitude").cast<double>();
                double NA_value = (NA * ureg.attr("dimensionless")).attr("to")(ureg.attr("dimensionless")).attr("magnitude").cast<double>();
                double optical_power_watt = (optical_power.attr("to")(ureg.attr("watt"))).attr("magnitude").cast<double>();

                new (&instance) Gaussian(wavelength_meter, jones_vector, NA_value, optical_power_watt);
            },
            py::arg("wavelength"),
            py::arg("jones_vector"),
            py::arg("NA"),
