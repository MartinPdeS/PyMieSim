    ...
    namespace Polarization {
        ...
        )
        {
            if (py::isinstance<PolarizationSet>(polarization)) {
                PolarizationSet polarization_set = polarization.cast<PolarizationSet>();

                if (polarization_set.size() == 1) {
                ...
                if (polarization_set.size() != target_size) {
                    throw std::invalid_argument(
                        "Inconsistent sizes: 'polarization' has size " +
                        std::to_string(polarization_set.size()) +
                        " but expected 1 or " +
                        std::to_string(target_size) + "."
                    );
                }

                return polarization_set;
            }
            ...
            throw std::invalid_argument(
                ...
                "a PolarizationState, a PolarizationSet, or a sequence of PolarizationState."
            );
        }
        ...
        MaterialSetType create_material_set_from_pyobject(
            ...
            if (target_size == 0) {
                throw std::runtime_error(
                    "Cannot broadcast '" + material_name + "' because target_size is 0."
                );
            }

            if (py::isinstance<MaterialSetType>(material_object)) {
                MaterialSetType material_set = py::cast<MaterialSetType>(material_object);

                if (material_set.size() == 1) {
                ...
                if (material_set.size() != target_size) {
                    throw std::runtime_error(
                        "Inconsistent sizes for " + material_name + ". MaterialSet has size " +
                        std::to_string(material_set.size()) + " but expected 1 or " +
                        std::to_string(target_size) + "."
                    );
                }
            ...
            if (py::isinstance<py::list>(material_object) || py::isinstance<py::tuple>(material_object)) {
                ...
                if (py::len(material_sequence) == 0) {
                    throw std::runtime_error(
                        "Invalid type for " + material_name + ". Empty sequences are not allowed."
                    );
                }
                ...
                if (materials.size() == 1) {
                    materials = std::vector<std::shared_ptr<BaseClass>>(target_size, materials[0]);
                }
                else if (materials.size() != target_size) {
                    throw std::runtime_error(
                        "Inconsistent sizes for " + material_name + ". Sequence has size " +
                        std::to_string(materials.size()) + " but expected 1 or " +
                        std::to_string(target_size) + "."
                    );
                }
