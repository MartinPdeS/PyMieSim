...
def get_laguerre_gaussian_mode_field(
        coords: numpy.ndarray,
        p: int,
        l: int,  # no E714  # noqa: E741
    ...
    # Laguerre polynomial
    L_pl = genlaguerre(p, abs(l))(2 * r**2 / w**2)
    amplitude = (numpy.sqrt(2) * r / w)**abs(l) * L_pl * numpy.exp(-r**2 / w**2)

    # Phase factor
    phase = l * theta - k * r**2 / (2 * R) + (2 * p + abs(l) + 1) * gouy_phase
...
def interpolate_from_structured_mesh(sampling: int = 50, **kwargs) -> numpy.ndarray:
    ...
    mode_field = get_laguerre_gaussian_mode_field(coordinate[:2, :] / norm, **kwargs)
