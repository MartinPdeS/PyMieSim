...
)
{
    ...
    size_t failed_index = total_iterations;
    ...
    for (long long idx_flat = 0; idx_flat < static_cast<long long>(total_iterations); ++idx_flat) {
        try {
            ...
            if (phi_field.size() != stride_channel || theta_field.size() != stride_channel) {
                ...
                message << "Far-field size mismatch at configuration " << idx_flat
                        << ": expected " << stride_channel << " samples per component, got "
                        << phi_field.size() << " phi and " << theta_field.size() << " theta samples.";
            ...
            // Exceptions must not escape an OpenMP worker. Report the lowest
            // failing configuration deterministically after all workers finish.
            #pragma omp critical(pymiesim_farfield_error)
            {
                if (static_cast<size_t>(idx_flat) < failed_index) {
                    failed_index = static_cast<size_t>(idx_flat);
