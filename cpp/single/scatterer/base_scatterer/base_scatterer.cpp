...
BaseScatterer::get_structured_total_nearfields(
    ...
    const int number_of_x_points = static_cast<int>(x_range.size());
    const int number_of_y_points = static_cast<int>(y_range.size());
    const int number_of_z_points = static_cast<int>(z_range.size());
    ...
    if (static_cast<size_t>(number_of_x_points) >
        std::numeric_limits<size_t>::max() / static_cast<size_t>(number_of_y_points)) {
